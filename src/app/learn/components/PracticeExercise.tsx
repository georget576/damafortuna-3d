...
export default function PracticeExercise({ difficulty = 'beginner' }: PracticeExerciseProps) {
  ...
  return (
    <div className="space-y-4 md:space-y-6 font-just-another-hand tracking-widest">
      ...
      {exerciseCards.length > 0 ? (
        <Card className="bg-gray-800/50 border-gray-700 text-purple-300">
          <CardHeader>
            {exerciseType === 'card' && (
              <CardTitle className="font-caveat-brush text-lg md:text-xl lg:text-2xl">Card Recognition Exercise</CardTitle>
            )}
            {exerciseType === 'decision' && (
              <CardTitle className="font-caveat-brush text-lg md:text-xl lg:text-2xl">Spread Selection Exercise</CardTitle>
            )}
            {exerciseType === 'meaning' && (
              <CardTitle className="font-caveat-brush text-lg md:text-xl lg:text-2xl">Card Meaning Exercise</CardTitle>
            )}
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-4 md:gap-6">
              <div>
                <div className="mb-4 md:mb-6">
                  {exerciseType === 'card' && (
                    <h3 className="font-bold mb-2 text-purple-300 text-sm md:text-base lg:text-lg">Select the correct card:</h3>
                  )}
                  {exerciseType === 'decision' && (
                    <h3 className="font-bold mb-2 text-purple-300 text-sm md:text-base lg:text-lg">Situation: {exerciseCards[0].name}</h3>
                  )}
                  {exerciseType === 'meaning' && (
                    <h3 className="font-bold mb-2 text-purple-300 text-sm md:text-base lg:text-lg">Primary meaning:</h3>
                  )}
                  <div className="space-y-2">
                    {exerciseOptions.map((option, index) => (
                      <Button
                        key={index}
                        variant="outline"
                        className="w-full justify-start text-left bg-gray-700/50 hover:bg-gray-600 border-gray-600 text-sm md:text-base lg:text-lg"
                ...
                <div className="flex gap-2">
                  <Button onClick={generateExercise} variant="outline" className='bg-purple-600 text-sm md:text-base lg:text-lg'>
                    Next Exercise
                  </Button>
                </div>
              </div>
              ...
              <div className="flex justify-center">
                {exerciseType === 'card' && (
                  <div className="w-32 h-48 sm:w-36 sm:h-54 md:w-40 md:h-60 bg-black rounded-lg overflow-hidden flex items-center justify-center">
                    <img
                ...
                {exerciseType === 'decision' && (
                  <div className="w-32 h-48 sm:w-36 sm:h-54 md:w-40 md:h-60 bg-black rounded-lg overflow-hidden flex items-center justify-center p-3 md:p-4">
                    <div className="text-center text-white">
                      <div className="text-3xl sm:text-4xl mb-2">❓</div>
                      <p className="text-xs sm:text-sm md:text-base text-center">Decision Making</p>
                    </div>
                  </div>
                )}
                {exerciseType === 'meaning' && (
            ...
            {showFeedback && (
              <div className={`mt-4 p-3 md:p-4 rounded-lg ${
                ...
                <h3 className="font-bold mb-2 text-sm md:text-base lg:text-lg">
                  {isCorrect ? 'Correct! 🎉' : 'Not quite right 🤔'}
                </h3>
                <p className="text-gray-300 text-xs md:text-base lg:text-xl">
                  {isCorrect
        ...
        <Card className="bg-gray-800/50 border-gray-700">
          <CardContent className="text-center py-8 md:py-12">
            <Lightbulb className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 mx-auto mb-3 md:mb-4 text-purple-400" />
            <p className="text-gray-400 text-sm md:text-base lg:text-xl">Click "New Exercise" to start practicing!</p>
          </CardContent>
        </Card>
      )}
    </div>
